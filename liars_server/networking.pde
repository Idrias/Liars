class Network {
  ArrayList<String> sendingQueue;
  ArrayList<String> kickQueue;
  ...
  void push(String command, String tag1, String tag2, boolean instantSending) {
    // This get called from outside: Either schedules a sending task or sends immediately.
    String message = command+"<"+tag1+"><"+tag2+">;";
    if (instantSending) server.write(message);
    else sendingQueue.add(message);
  }

  void send() {
    // Send the messages that have been stored in sendingQueue
    for (String msg : sendingQueue) {
      server.write(msg);
    }
  }
    ...
    if (command.equals("+gst")) {
      ...
      while (getById(newPlayerS).cards.size()==0) {
        ...
        if (winner.equals("nowinner")) {
          winner = "a winner"; 
          server.write("+msg<><"+getById(newPlayerS).alias+", HAT GEWONNEN!>;");
        }
      ...
      server.write("+msg< >< >;");
    } else if (command.equals("+paa")) {
      playingas = tag1; 
      server.write("+paa<"+tag1+"><>;");
    } else if (command.equals("+lam")) {
      for (Player player : players) {
        if (player.theirturn) server.write("+msg<"+player.alias+">< glaubt: Lüge!>;");
      }
      for (Server_Card card : lastcards) {
        server.write("+tsc<"+card.farbe+"><"+card.id+">;");
        if (!card.id.equals(playingas)) {
          lied = true; 
          server.write("+pll<"+tag1+"><>;");
        }
